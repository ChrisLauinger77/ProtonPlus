namespace ProtonPlus.Widgets.MangoHud {
    public class VisualPage : Page {
        ...
        public VisualPage (Utils.MangoHudManager config) {
            ...
            bg_color_row.add_prefix (new Gtk.Image.from_icon_name ("fill-color-symbolic"));
            ...
            columns_row = create_combo (
                _ ("Columns"),
                {"1", "2", "3", "4", "5", "6"},
                config.table_columns > 0 ? config.table_columns - 1 : 0,
            ...
            toggle_hud_row = create_combo (
                _ ("HUD Toggle Key"),
                {"Shift_R+F12", "Shift_R+F1", "Shift_R+F2", "Shift_R+F3", "Shift_R+F4", _ ("None")},
                0,
